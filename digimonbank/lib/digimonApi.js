...
export async function getDigimon(nameOrId) {
  try {
    const res = await fetch(`${BASE_URL}/digimon/${nameOrId}`, {
...
export async function getDigimonList(page = 1, pageSize = 20) {
  try {
    const res = await fetch(
      `${BASE_URL}/digimon?page=${page}&pageSize=${pageSize}`,
      { cache: "no-store" }
    );
    ...
    const data = await res.json();
    return data.content || data; // depende do formato da API
  } catch (err) {
...
// Buscar detalhes de um Digimon por ID (opcional)
export async function fetchDigimonById(id) {
  try {
    const res = await fetch(`${BASE_URL}/digimon/${id}`, {
      cache: "no-store"
    });
    if (!res.ok) throw new Error("Erro ao buscar Digimon por ID");
